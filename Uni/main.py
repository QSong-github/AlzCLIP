"""
BRAIN CLIP training function uses a mixed expression of SNPs and labels.
"""
import os
...
dataset = CLIPDatasetUni(args,dataset_path='./snp_lbl_img_dataset_ova')
...
# Randomly split the dataset into proportions.
...
def train_epoch(model, train_loader, optimizer):
    ...
    for batch in tqdm_object:
        batch = {k: v.cuda() for k, v in batch.items() if k == "img" or k == "lbl_snp" or k=='gt'}
        loss = model(batch)
        optimizer.zero_grad()
        loss.backward()
...
def main():
    ...
    for epoch in range(args.max_epochs):
        ...
        model.eval()
        # The model performance is judged by the test set.
        with torch.no_grad():
        ...
        if test_loss.avg < best_loss:
            ...
            torch.save(model.state_dict(), str(args.save_path) + "/best128.pt")

...
def get_snp_embeddings(loader, model):
    ...
    return torch.cat(_snp_embeddings)

def get_lbl_embeddings(loader, model):
    model.eval()
    _lbl_embeddings = []
    _lbl = []
    with torch.no_grad():
        for batch in tqdm(loader):
            ...
            lbl_features = model.lbl_encoder(batch['lbl'].unsqueeze(1).cuda())
            lbl_embeddings = model.lbl_projection(lbl_features)
            _lbl_embeddings.append(lbl_embeddings)

    return torch.cat(_lbl_embeddings), torch.cat(_lbl)
...
def model_infer():
    ...
    lbl_embeddings_all, labels_all = get_lbl_embeddings(loader_all, model)

    img_embeddings_all = img_embeddings_all.cpu().numpy()
    lbl_embeddings_all = lbl_embeddings_all.cpu().numpy()
    labels_all = labels_all.cpu().numpy()
    print(img_embeddings_all.shape)
    print(lbl_embeddings_all.shape)
    ...
    lbl_embeddings = lbl_embeddings_all[:refer_size]
    labels = labels_all[:refer_size]

    print(img_embeddings.shape)
    print(lbl_embeddings.shape)
    print(labels.shape)
    np.save(save_path + "img_embeddings_refer.npy", img_embeddings.T)
    np.save(save_path + "lbl_embeddings_refer.npy", lbl_embeddings.T)
